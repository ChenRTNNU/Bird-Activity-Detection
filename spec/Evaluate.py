...
def create_augmented_batch(data, targets, t, f):
    ...
    for i in range(batch_size):
        ...
        # 对选择的区域进行算数平均
        augmented_data[i, :, start_time:start_time + t, :] = 0

        augmented_data[i, :, :, start_freq:start_freq + f] = 0
...
def specaug(data, targets):
    ...
    for i in range(0, batch_size):
        ...
        augmented_data[i] = time_masking(frequency_masking(data[i]))
...
def train(model, data_loader, optimizer, scheduler, device, epoch, epochs):
    ...
    for i, (mels, labels) in enumerate(loop):
        ...
        mels, labels = specaug(mels, labels)
        # print(mels[0])
        # print(labels.shape)
        # mels,labels=specaug(mels,labels)
        # print(mels[0])
