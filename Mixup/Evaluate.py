...
def create_augmented_batch(data, targets):
    ...
    for i in range(batch_size):
        ...
        if label == 1:
            ...
            chosen_targets = targets[negative_indices[chosen_index]]
        ...
        # 对选择的区域进行算数平均
        augmented_data[i, :, :, :] = u*data[i, :,:, :] + (1-u)*chosen_data[:,:,:]
        augmented_targets[i] = u*targets[i] + (1-u)*chosen_targets# print(targets[index],targets[i])
...
def train(model, data_loader, optimizer, scheduler, device, epoch, epochs):
    ...
    for i, (mels, labels) in enumerate(loop):
        ...
        mels, labels = create_augmented_batch(mels,labels)
    ...
    return running_loss / len(data_loader), model
